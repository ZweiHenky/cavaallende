                        ...
                        <View className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-8">
                            ...
                            <View className="flex-row justify-between items-center mb-2">
                                ...
                                <Text className="text-base text-primary font-medium">
                                    {formatterCurrency(Number(order.subtotal))}
                                </Text>
                            </View>
                            
                            {Number(order.shipping_cost) > 0 && <View className="flex-row justify-between items-center mb-2">
                                <Text className="text-base text-gray-400">Shipping cost</Text>
                                <Text className="text-base text-primary font-medium">
                                    {formatterCurrency(Number(order.shipping_cost))}
                                </Text>
                            </View>}

                            {Number(order.taxes) > 0 && <View className="flex-row justify-between items-center mb-2">
                                <Text className="text-base text-gray-400">Taxes (IVA)</Text>
                                <Text className="text-base text-primary font-medium">
                                    {formatterCurrency(Number(order.taxes))}
                                </Text>
                            </View>}
                            ...
                            {Number(order.discount) > 0 && (
                                <View className="flex-row justify-between items-center mb-2">
                                    ...
                                    <Text className="text-base text-green-600 font-medium">
                                        - {formatterCurrency(Number(order.discount))}
                                    </Text>
                                </View>
                            )}

                            <View className="flex-row justify-between items-center mt-3 pt-3 border-t border-gray-200">
                                <Text className="text-xl font-bold text-tertiary">Final total</Text>
                                <Text className="text-2xl font-bold text-primary">
                                    {formatterCurrency(Number(order.total))}
                                </Text>
                            </View>
                        </View>
